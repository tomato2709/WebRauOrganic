...
namespace WebRauTNT.Controllers
{
    public class DonHangController : Controller
    {
        ...
        public ActionResult LSMua()
        {
            ...
            {
                var makh = User.Identity.GetUserId();
                List<Models.LinQ.DonHang> donhangdamua = data.DonHangs.Where(a => a.AspNetUser.Id == makh).ToList();
                /*List<Models.LinQ.ChiTietDonHang> ctdh = data.ChiTietDonHangs.Where(c => c.MaDH == donhangdamua.MaDH).ToList();*/
                var viewDM = new DMViewModel()
                {
        ...
        public ActionResult Create([Bind(Include = "MaDH,TrangThaiGiaoHang,NgayDat,NgayGiao,MaKH,NguoiNhan,DiaChiGiaoHang,SĐTNguoiNhan")] Models.DonHang donHang)
        {
            if (!AuthAdmin())
                return RedirectToAction("Error401", "Admin");
